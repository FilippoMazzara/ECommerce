<?xml version = "1.0"?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN"
"http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">
<html xmlns = "http://www.w3.org/1999/xhtml">
<head>
        <meta charset="UTF-8" />
        <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
        <meta name="login" content="width=device-width, initial-scale=1.0">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Il Mercatino</title>
        <link rel="stylesheet" type="text/css" href ="<%= request.getContextPath() %>/profilo/style.css"/>
        <link rel="stylesheet" href="<%= request.getContextPath() %>/css/bootstrap.min.css">
        <link rel="stylesheet" href="<%= request.getContextPath() %>/css/style.css">

        <style>

              body{
                 background: linear-gradient(to bottom, rgba(255, 255, 255, 0.890) 0%, rgb(255, 255, 255) 100%), url("<%= request.getContextPath() %>/img/sfondo.jpg");
                 padding-bottom: 50px;
                }

        </style>

       
</head>
<body>

     <!--Navbar-->
    <jsp:include page = "/navbar" />



    <!--Ripilogo ordine-->
    
    <div class=" bg-white container text-center py-4" style="width:800px; margin:0 auto;">
    <img style="height:100px; width:100px;" src="<%= request.getContextPath() %>/img/conferma.png"> <h1>Ordine confermato</h1>

    
    </div>





    
        


</body>
</html>